...
def get_tasks():
    ...
    c = conn.cursor()
    c.execute('''
        SELECT task_uuid, task, task_priority, task_category, is_completed
        FROM tasks
        WHERE user_uuid = ?
          AND task_date = ?
...
def add_task():
    ...
    conn = sqlite3.connect('deeptodo.db')
    c = conn.cursor()
    c.execute("INSERT INTO tasks (task_uuid, user_uuid, task, task_priority, task_category, task_date) VALUES (?, ?, ?, ?, ?, ?)",
             (task_uuid, user_uuid, data['task'], data['task_priority'], data['task_category'], data['date']))
    conn.commit()
    conn.close()
    return jsonify({"task_uuid": task_uuid, "task": data['task'], "task_priority": data['task_priority'], "task_category": data['task_category'], "completed": False}), 201
