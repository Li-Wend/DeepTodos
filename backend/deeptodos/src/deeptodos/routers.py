...
def include_todo_routers(app: FastAPI) -> None:
    ...
    async def create_task(
        task: schemas.TaskCreate, db: AsyncSession = Depends(get_async_session)
    ):
        """创建新任务"""
        return await helper.task_crud.create_task(db, task)

    @app.get(
        "/deeptodos/read-tasks", response_model=List[schemas.Task], tags=["deeptodos"]
    )
    ...
    @app.get(
        "/deeptodos/read-tasks-summary",
        response_model=List[schemas.TaskSummary],
        tags=["deeptodos"],
    )
    ...
    @app.post(
        "/deeptodos/complete-task/{task_id}",
        response_model=schemas.Task,
        tags=["deeptodos"],
    )
