...
from src.authentication.users import User, auth_backend, fastapi_users, current_active_user
...
def include_auth_routers(app: FastAPI) -> None:
    ...
    app.include_router(
        fastapi_users.get_register_router(UserRead, UserCreate), prefix="/auth", tags=["authentication"]
    )
    app.include_router(fastapi_users.get_reset_password_router(), prefix="/auth", tags=["authentication"])
    app.include_router(fastapi_users.get_verify_router(UserRead), prefix="/auth", tags=["authentication"])
    app.include_router(
